...
const getProyecto = (proyecto) => `
  ...
  <div>
    ...
    <p>Tecnologías usadas en el proyecto: ${getTecnologias(proyecto.tecnologias)}</p>
  </div>
...
const makeEstudios = () => {
  const nuevoEstudios = document.createElement("div");
  nuevoEstudios.classList = "Estudios";
  informacionPersonal.Estudios.forEach(dato => {
    nuevoEstudios.innerHTML += `
    <div>
      <span>${dato[0]}:</span>
      <span>${dato[1]}</span>
    </div>
  ...
  aside.appendChild(nuevoEstudios);
}
